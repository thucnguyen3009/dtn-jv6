	...
	public String postCreateAddress(Model m, @RequestParam("newAddress") Optional<String> address,
		...
		if (address.isPresent() && acc != null) {
			...
			if (!address.get().isBlank()) {
				...
				return "redirect:/update_account.html?tb=errorAdrNull#messageAdr";
			}
			adr.setUserAr(acc);
		...
		return "redirect:/update_account.html?tb=successAdr#messageAdr";
	}
		...
		if (id.isPresent()) {
			try {
				...
				if (adr.isPresent() && adr.get().getUserAr().getUsername().equals(acc.getUsername())) {
					if (list.size() == 1) {
						return "redirect:/update_account.html?tb=error#messageAdr";
					} else {
						addressService.delete(ma);
						return "redirect:/update_account.html?tb=success#messageAdr";
					}
				} else {
					return "redirect:/update_account.html?tb=error#messageAdr";
				}
